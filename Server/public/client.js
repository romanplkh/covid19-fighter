const { RANDOM_NERDS_SERVER, RANDOM_NERDS_CLIENT } = {
  RANDOM_NERDS_SERVER: "randomnerdsServer",
  RANDOM_NERDS_CLIENT: "randomnerdsClient",
};
...
registerPlayerBtn.addEventListener("click", () => {
  ...
  if (!username) {
    ...
    //CREATE CLIENT
    const cl = new Paho.MQTT.Client(
      "mga.twilightparadox.com",
      Number(8083),
      username
    );
...
function onConnect() {
  ...
  sendMessageObject(
    {
      action: ON_USER_JOIN,
      payload: { userId: client.clientId, userScore: 0 },
    },
    RANDOM_NERDS_SERVER
  );
...
function showVirus() {
  if (gameState.gameIsPlaying) {
    sendMessageObject(
      { action: GET_RANDOM_BUSH, payload: bushes.length },
      RANDOM_NERDS_SERVER
    );
...
function getScore() {
  sendMessageObject(
    { action: CALCULATE_SCORE, payload: { userId: client.clientId } },
    RANDOM_NERDS_SERVER
  );
...
function displayPlayerScore(id, score) {
  if (id == client.clientId) playerScoreText.innerHTML = score;
}
...
function toggleError(show, error) {
  ...
  if (show) {
    ...
    errorAlert
      .querySelector("span")
      .removeEventListener("click", hideErrorClick);
  }
}
...
window.addEventListener("beforeunload", () => {
  if (client) {
    sendMessageObject(
      { action: ON_USER_LEAVE, payload: client.clientId },
      RANDOM_NERDS_SERVER
    );
  }
});
